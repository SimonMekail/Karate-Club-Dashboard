...
const EditModal = ({
  ...
  show,
  onHide,
  successMessage = `تم تعديل ${itemType} بنجاح`,
  errorMessage = `حدث خطأ أثناء تعديل ${itemType}`,
  additionalData = {},
}) => {
  const { user } = AuthData();
  ...
  const handleSubmit = (e) => {
    ...
    const toastId = toast.loading(`جاري تعديل ${itemType}...`);
    setIsEditing(true);
    setErrors({});
    ...
    axios
      .put(`${apiEndpoint}${itemId}`, requestData, {
        headers: {
      ...
      case "date":
        return (
          <Col key={field.name} md={6} className={field.colClassName}>
            <Form.Group controlId={`edit-${field.name}`} className="mb-3">
              {commonField}
              <Form.Control
                type={field.type}
                {...fieldProps}
  ...
  return (
    <Modal
      ...
      onHide={onHide}
      size={modalSize}
      aria-labelledby={`EditModalLabel-${itemType}`}
      backdrop="static"
        ...
        <div className="d-flex align-items-center">
          ...
          <div>
            ...
            <p className="small mb-0 opacity-75" style={{ color: "white" }}>
              قم بتعديل الحقول التالية لتحديث {itemType}
            </p>
          </div>
        </div>
      </Modal.Header>
